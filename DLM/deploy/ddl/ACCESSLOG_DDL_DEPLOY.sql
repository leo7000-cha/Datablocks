...
CREATE TABLE IF NOT EXISTS COTDL.TBL_ACCESS_LOG_SOURCE (
    ...
    source_type       VARCHAR(20)   DEFAULT 'DB_AUDIT' COMMENT '수집 방식 (DB_AUDIT, GENERAL_LOG, DLM_SELF)',
...
CREATE TABLE IF NOT EXISTS COTDL.TBL_ACCESS_LOG (
    ...
    INDEX idx_al_access_time (access_time, source_system_id),
    INDEX idx_al_user_account (user_account, access_time),
    INDEX idx_al_target_table (target_db, target_schema, target_table, access_time),
...
CREATE TABLE IF NOT EXISTS COTDL.TBL_ACCESS_LOG_ALERT (
    ...
    status            VARCHAR(20)   DEFAULT 'NEW' COMMENT '상태 (NEW/ACKNOWLEDGED/RESOLVED/DISMISSED)',
...
(UUID(), 'SQL_TEXT_LOGGING',          'N',    'GENERAL',   'SQL 전문 기록 여부',                 'Y',  2, 'system'),
-- 수집
...
(UUID(), 'EMAIL_ENABLED',            'N',    'ALERT',     '이메일 알림 활성화 (Y/N)',             'Y', 21, 'system'),
