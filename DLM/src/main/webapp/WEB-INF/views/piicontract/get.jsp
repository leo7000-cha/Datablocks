            ...
            <div class="col-sm-12">
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <form style="margin: 0; padding: 0;" role="form" id="piiconfig_get_form" method="post">
                            <table class="m-1" style="border-collapse: collapse; border: 1px; width: 70%">
                                ...
                                <tr>
                                    <th class="th-get"><spring:message code="col.cfgkey" text="Key"/></th>
                                    <td class="td-get-l" colspan=2>
                                        <c:out value="${piiconfig.cfgkey}"/><input type="hidden"
                                                                                   class="form-control form-control-sm"
                                                                                   name='cfgkey'
                                                                                   value='<c:out value="${piiconfig.cfgkey}"/>'>
                                    </td>
                                </tr>
                                <tr>
                                    <th class="th-get"><spring:message code="col.value" text="Value"/></th>
                                    <td class="td-get-l" colspan=2>
                                        <c:set var="cfgkey" value="${piiconfig.cfgkey}"/>
                                        <c:choose>
                                            <c:when test="${fn : contains(cfgkey, '_FLAG')}">
                                                <c:choose>
                                                    <c:when test="${piiconfig.value eq 'Y'}"> <input type="checkbox"
                                                                                                     name='value'
                                                    ...
                                                    <c:when test="${piiconfig.value eq 'N'}"> <input type="checkbox"
                                                                                                     name='value'
                                                                                                     value='<c:out value="${piiconfig.value}"/>'
                                                                                                     data-toggle="toggle"
                                                                                                     data-size="xs"
                                                                                                     disabled> </c:when>
                                                    <c:otherwise> <c:out value="${piiconfig.value}"/> </c:otherwise>
                                                </c:choose>
                                            </c:when>
                                            <c:otherwise>
                                                <c:out value="${piiconfig.value}"/><input type="hidden"
                                                                                          class="form-control form-control-sm"
                                                                                          name='comments'
                                                                                          value='<c:out value="${piiconfig.value}"/>'>
                                            </c:otherwise>
                                ...
                                <tr>
                                    ...
                                    <td class="td-get-l" colspan=2>
                                        <c:out value="${piiconfig.comments}"/><input type="hidden"
                                                                                     class="form-control form-control-sm"
                                                                                     name='comments'
...
<script type="text/javascript">
    $(function () {
        $("#menupath").html(Menupath + '<i class="fas fa-chevron-right" style="font-size: 18px; margin: 0 6px; color: #888;"></i>' + "<spring:message code="memu.control_management" text="Env configration"/>" + ">Details");
    });
    $(document).ready(function () {
        ...
        $("button[data-oper='modify']").on("click", function (e) {
            ...
            var serchkeyno = $('#piiconfig_get_form [name="cfgkey"]').val();
            ...
            url_view = "modify?cfgkey=" + serchkeyno + "&";//alert("/piiconfig/"+url_view+"pagenum="+pagenum+"&amount="+amount+url_search);
            ...
            //alert("/piiconfig/"+url_view+"pagenum="+pagenum+"&amount="+amount+url_search);
            $.ajax({
                type: "GET",
                url: "/piiconfig/" + url_view + "pagenum=" + pagenum + "&amount=" + amount + url_search,
                dataType: "text",
