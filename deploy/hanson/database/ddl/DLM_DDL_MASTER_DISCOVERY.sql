...
CREATE TABLE COTDL.TBL_DISCOVERY_SCAN_RESULT (
    ...
    -- 암호화 상태
    encryption_status VARCHAR(20)   DEFAULT 'NONE' COMMENT '암호화 상태 (NONE, HASHED, ENCRYPTED, UNKNOWN)',
    encryption_method VARCHAR(50)   DEFAULT NULL COMMENT '탐지된 암호화 방법 (SHA-256, MD5, BCrypt, AES/Base64 등)',
    encryption_ratio  INT           DEFAULT 0 COMMENT '암호화 비율 (0-100%, 샘플 중 암호화된 값의 비율)',
    ...
    INDEX idx_result_encryption_status (encryption_status)
...
CREATE TABLE COTDL.TBL_DISCOVERY_PII_REGISTRY (
    ...
    -- Encryption Info
    encryption_status          VARCHAR(20)    DEFAULT 'NONE' COMMENT 'Encryption status (NONE, HASHED, ENCRYPTED, UNKNOWN)',
    encryption_method          VARCHAR(50)    DEFAULT NULL COMMENT 'Detected encryption method (SHA-256, MD5, BCrypt, AES/Base64 etc)',
    encryption_ratio           INT            DEFAULT 0 COMMENT 'Encryption ratio (0-100%)',
...
('ENCRYPTED_PII',     '암호화 PII', 'Encrypted PII (Unidentified)', 'SENSITIVE', '암호화/해시 처리된 개인정보 - 유형 판별 불가', NULL,                   900),
