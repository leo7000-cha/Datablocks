# DLM 배포 가이드 — 한국손사 (DEV: Docker 컨테이너 MariaDB)

> 폐쇄망 Ubuntu / Docker 설치됨 / MariaDB 컨테이너 이미 운영 중
> 작성일: 2026-04-08 | 갱신: 2026-04-22

> 본 문서는 개발(DEV) 환경용입니다. MariaDB 가 **호스트 OS 에 직접 설치**된
> 운영(PROD) 환경은 [`../hanson_prod/README-한국손사.md`](../hanson_prod/README-한국손사.md) 를 참조하세요.
...
├── mariadb/
│   ├── DLM_DATABASE_INIT.sql          ← DB/계정 초기화 SQL
│   ├── cotdl_dump.sql.data            ← DB 스키마 + 데이터
│   ├── custom-prod.cnf                ← MariaDB 권장 설정
...
> MariaDB가 Docker 컨테이너이므로 `docker exec`로 실행합니다.

### 패치 파일을 mariadb 컨테이너로 복사 후 실행

```bash
# 1. 패치 파일을 컨테이너로 복사
docker cp mariadb/DLM_DDL_MASTER_ACCESSLOG.sql mariadb:/tmp/
docker cp mariadb/DLM_DDL_MASTER_DISCOVERY.sql mariadb:/tmp/

# 2. 패치 실행 (순서 중요!)
docker exec -i mariadb mysql -u cotdl -p'!Dlm1234' cotdl < mariadb/DLM_DDL_MASTER_ACCESSLOG.sql
docker exec -i mariadb mysql -u cotdl -p'!Dlm1234' cotdl < mariadb/DLM_DDL_MASTER_DISCOVERY.sql

# 고객사별 패치가 있는 경우:
# docker exec -i mariadb mysql -u cotdl -p'!Dlm1234' cotdl < mariadb/patches/PATCH_파일.sql
```
...
```bash
# 핵심 신규 테이블 확인
docker exec mariadb mysql -u cotdl -p'!Dlm1234' cotdl -e "SHOW TABLES LIKE 'tbl_access_log%';"
docker exec mariadb mysql -u cotdl -p'!Dlm1234' cotdl -e "SHOW TABLES LIKE 'tbl_discovery%';"

# 테이블 수 확인
docker exec mariadb mysql -u cotdl -p'!Dlm1234' cotdl -e "SELECT COUNT(*) AS table_count FROM information_schema.tables WHERE table_schema='cotdl';"
```
...
```bash
# mariadb 컨테이너에서 SQL 실행
docker exec -i mariadb mysql -u cotdl -p'!Dlm1234' cotdl < /tmp/patch/PATCH_YYYYMMDD_설명.sql
```
...
| compose 에러 | MARIADB_NETWORK 미설정 | `--env-file .env.hanson` 옵션 확인 |
