# 접속기록 AI 분석 엔진 — X-One Privacy-AI 확장 설계서
...
다양한 채널(DLM Agent, Application SDK, DB Audit Log)로 수집된 접속기록(TBL_ACCESS_LOG)을 DLM-Privacy-AI에서 AI 기반으로 분석하여 **이상행위 탐지**, **사용자 위험도 스코어링**, **접근 패턴 요약**을 수행한다.
...
┌────────────────┐    HTTP POST    ┌──────────────────┐          ┌──────────────────┐
│ DLM Java Agent │ ──────────────► │ AgentApiController│          │ PII 탐지만 수행   │
│ (ByteBuddy BCI)│  /api/agent/logs│ → TBL_ACCESS_LOG │          │ (detect API)     │
└────────────────┘                 └──────────────────┘          └──────────────────┘
```

- **DLM Agent**: WAS JVM에 `-javaagent`로 주입, JDBC 바이트코드 가로채기로 SQL 캡처 → 3초마다 500건 배치 전송
...
```
┌──────────────────────────────────────────────────────────────────────────┐
│  채널 A: DLM Agent (BCI)           채널 B: App SDK            채널 C    │
│  ┌────────────────┐    HTTP POST   ┌──────────────┐   DB Audit│         │
...
### 2.1 채널 A: DLM Agent (BCI 방식) — 이미 구현 완료
...
### 2.4 채널 조합 전략: 전체 접근 기록 커버리지

단일 채널로는 DB에 대한 모든 접근 경로를 커버할 수 없다. **채널 A(BCI) + 채널 C(DB 접근제어 연동)** 조합이 최적의 커버리지를 제공한다.

```
┌─ 채널 A: BCI Agent ──────────────────────────┐
│  WAS 경유 트래픽 100% 캡처                     │
│  → 웹 사용자(홍길동) + SQL + 정보주체 전부 확보  │
└──────────────────────────────────────────────┘
        +
┌─ 채널 C: DB 접근제어 연동 ───────────────────┐
│  DBA 직접 접속 로그만 가져옴                    │
...
| 접근 경로 | 담당 채널 | 사용자 식별 | 비고 |
|---|---|---|---|
| 웹 사용자 (WAS 경유) | **채널 A** (BCI) | O | 커넥션 풀 뒤의 실사용자 식별 가능 |
| DBA 직접 접속 | **채널 C** (DB 접근제어 연동) | O | DB 접근제어 솔루션이 실명 인증 수행 |
| 배치/ETL | 채널 C에서 IP/계정 정도 | △ | 시스템 계정이므로 "무엇을 얼마나" 수준 감시로 충분 |

> **참고: DB 접근제어 솔루션과 WAS 경유 트래픽의 한계**
>
> DB 접근제어 솔루션(DBSafer, Chakra Max 등)은 DBA 직접 접속 시 실명 기반 인증을 수행하므로 "누가"를 알 수 있다.
> 그러나 WAS가 커넥션 풀로 DB에 접속하는 경우, DB 접근제어 솔루션 입장에서는 WAS의 공용 DB 계정(예: `APP_USER`)만 보이므로
> 실제 웹 사용자(홍길동/김철수)를 구분할 수 없다. 이것이 채널 A(BCI)가 WAS 경유 트래픽에 필수인 이유다.

**결론**: 채널 A(BCI) + 채널 C(DB 접근제어 DBA 로그) 조합이면 ISMS-P 심사에서 요구하는
"**누가 언제 누구의 정보에 접근했는가**"를 빈틈 없이 답할 수 있는 구조가 된다.
채널 B(SDK)는 BCI 설치가 불가능한 비-Java 환경을 위한 대안 채널로 선택적으로 추가한다.

### 2.5 access_channel 컬럼 값 정의

| 값 | 수집 채널 | 설명 |
|----|----------|------|
| `WAS_AGENT` | 채널 A | DLM Java Agent (BCI) |
...
## 3. 채널 A 상세: DLM Agent BCI 동작 원리 (구현 완료)
...
| 고객 유형 | 권장 채널 | 설득 포인트 |
|----------|----------|-----------|
| BCI 수용 가능 | 채널 A (DLM Agent) | 100% 누락 없음, 정보주체 식별, ISMS-P 완벽 대응 |
