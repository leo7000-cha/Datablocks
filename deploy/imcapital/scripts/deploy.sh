...
mkdir -p "$INSTALL_DIR"
cp "$DEPLOY_ROOT/docker-compose.imcapital.yml" "$INSTALL_DIR/"
cp "$DEPLOY_ROOT/.env.imcapital" "$INSTALL_DIR/"
log "파일 복사 완료: $INSTALL_DIR"

ENV_FILE="$INSTALL_DIR/.env.imcapital"
...
cd "$INSTALL_DIR"
docker compose -f docker-compose.imcapital.yml down 2>/dev/null || true
...
echo ""
echo "  관리 명령어:"
echo "    시작:    cd $INSTALL_DIR && docker compose -f docker-compose.imcapital.yml up -d"
echo "    중지:    cd $INSTALL_DIR && docker compose -f docker-compose.imcapital.yml down"
