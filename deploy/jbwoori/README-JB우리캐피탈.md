...
> 폐쇄망 Rocky Linux 9 / Docker 미설치 / MariaDB 설치됨
> 작성일: 2026-04-08
...
├── .env.jbwoori                   ← JB우리캐피탈 전용 환경변수 (★ 수정 대상)
├── mariadb/
...
> `.env.jbwoori`의 `PRIVACY_AI_DB_PASSWORD` 값이 반드시 일치해야 합니다.

수정 후 반영:
```bash
cd /app/Datablocks
docker compose -f docker-compose.jbwoori.yml down
...
# 재시작 (DLM만)
docker compose -f docker-compose.jbwoori.yml restart dlm
...
```bash
cat >> ~/.bashrc << 'EOF'
alias dlm-up='cd /app/Datablocks && docker compose -f docker-compose.jbwoori.yml up -d'
alias dlm-down='cd /app/Datablocks && docker compose -f docker-compose.jbwoori.yml down'
alias dlm-restart='cd /app/Datablocks && docker compose -f docker-compose.jbwoori.yml restart'
...
| 로그인 후 에러 | DB 연결 실패 | `.env.jbwoori`의 DB IP 확인 |
