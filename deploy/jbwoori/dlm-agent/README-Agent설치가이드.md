# DLM Agent 설치 가이드 — JB우리캐피탈

> 대상: JB우리캐피탈 처리계 WAS 서버
> 작성일: 2026-04-14
...
#    dlm.agent.id=JBWOORI_WAS1                 ← 이 WAS 식별자
...
WAS별 설정 위치:
Tomcat → CATALINA_OPTS 또는 setenv.sh
WebLogic → 관리콘솔 > 서버 > 서버시작 > 인수
JEUS → jeus-jvm-option 또는 WebAdmin > JVM 옵션
Spring Boot → java -javaagent:... -jar app.jar
...
[XAudit-Agent] ========================================
[XAudit-Agent] Config loaded: serverUrl=http://192.168.1.100:8080, agentId=JBWOORI_WAS1
...
| `dlm.agent.id` | `JBWOORI_WAS1` | 이 Agent 식별자 (소스 등록 시 Agent ID와 일치) |
...
```bash
# WAS1
dlm.agent.id=JBWOORI_WAS1

# WAS2
dlm.agent.id=JBWOORI_WAS2
```
