#!/bin/bash
# ==============================================================================
# Docker 오프라인 설치 스크립트 — JB우리캐피탈 (Rocky Linux 9)
# 필요 권한: root 또는 sudo
# ==============================================================================
set -uo pipefail
...
echo ""
echo "=============================================="
echo "  Docker 오프라인 설치 — JB우리캐피탈 (Rocky 9)"
echo "=============================================="
echo ""
...
dnf --disablerepo="*" remove -y docker docker-client docker-common docker-engine docker-latest \
...
# --- 의존성 + Docker 설치 (Rocky 9 minimal 대응) ---
# 설치 순서: 의존성 → container-selinux → containerd → cli → ce → compose
log "RPM 설치 중 (의존성 포함 14개)..."

# STEP 1: 기본 의존성 (Rocky 9 minimal에 없을 수 있는 패키지 전부)
for pkg in gzip tar xz which iproute net-tools iptables-libs iptables-nft libseccomp; do
